...
with st.expander("Task & Evaluation"):
    st.markdown(
        "Design a BI dashboard with clean data, derived KPIs, sensible visuals, and actionable "
        "insights. Deliver as an interactive app with CSV and PDF export."
    )
...
missing = [k for k, p in expected.items() if not os.path.exists(p)]
if missing:
...
st.write("High-level KPIs for the selected slice.")
...
st.write("Use smoothing and chart type controls in the sidebar.")
...
# safe text helper for PDF (Latin-1 safe)
def _safe_text(x):
    if x is None:
        return ""
    s = str(x)
...
def build_pdf_bytes(merged_df, camp_df, spend_spikes_df, low_roas_df):
    ...
    pdf.cell(0, 8, _safe_text("Marketing Intelligence - Summary"), ln=True)
    pdf.ln(4)
    pdf.set_font("Helvetica", size=10)
    pdf.cell(0, 6, _safe_text(f"Date range: {merged_df['date'].min()} to {merged_df['date'].max()}"), ln=True)
    pdf.cell(0, 6, _safe_text(f"Total Revenue: ${merged_df['total_revenue'].sum():,.0f}"), ln=True)
    pdf.cell(0, 6, _safe_text(f"Marketing Spend: ${merged_df['spend'].sum():,.0f}"), ln=True)
    roas_val = merged_df['attributed_revenue'].sum() / (merged_df['spend'].sum() or np.nan)
    pdf.cell(0, 6, _safe_text(f"ROAS: {roas_val:.2f}" if not np.isnan(roas_val) else "ROAS: N/A"), ln=True)
    pdf.ln(6)
    pdf.set_font("Helvetica", size=11)
    pdf.cell(0, 6, _safe_text("Top campaigns (by spend)"), ln=True)
    pdf.set_font("Helvetica", size=9)
    if camp_df is not None and not camp_df.empty:
        top = camp_df.sort_values("spend", ascending=False).head(8)
        for _, row in top.iterrows():
            line = f"- {str(row.get('campaign',''))[:60]} | {row.get('channel','')} | spend: ${row.get('spend',0):,.0f} | ROAS: {row.get('roas',np.nan):.2f}"
            pdf.multi_cell(0, 5, _safe_text(line))
    else:
        pdf.cell(0, 5, _safe_text("No campaign data available."), ln=True)
    pdf.ln(6)
    pdf.set_font("Helvetica", size=11)
    pdf.cell(0, 6, _safe_text("Anomalies Detected"), ln=True)
    pdf.set_font("Helvetica", size=9)
    if not spend_spikes_df.empty:
        pdf.cell(0, 5, _safe_text("Spend spikes:"), ln=True)
        for _, r in spend_spikes_df.head(8).iterrows():
            pdf.multi_cell(0, 5, _safe_text(f"- {r['date']} | {r['channel']} | spend: ${r['spend']:,.0f}"))
    else:
        pdf.cell(0, 5, _safe_text("No spend spikes detected."), ln=True)
    if not low_roas_df.empty:
        pdf.ln(2)
        pdf.cell(0, 5, _safe_text("Low ROAS days:"), ln=True)
        for _, r in low_roas_df.head(8).iterrows():
            pdf.multi_cell(0, 5, _safe_text(f"- {r['date']} | spend: ${r['spend']:,.0f} | roas: {r['roas']:.2f}"))
    else:
        pdf.cell(0, 5, _safe_text("No low-ROAS days detected."), ln=True)
